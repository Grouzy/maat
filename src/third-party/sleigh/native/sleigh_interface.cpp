...
public:
    ...
    void setData(ghidra::uintb ad, const unsigned char *ptr, ghidra::int4 sz)
    {
        m_baseaddr = ad;
        m_data = ptr;
        m_length = sz;
...
public:
    ...
    void dump(const ghidra::Address &addr, const std::string &mnem, const std::string &body)
    {
...
public:
    ...
    )
    {
                ...
                (!max_instructions || (inst_count++ < max_instructions))) 
        {
            try
            {
                ...
                ghidra::int4 ilen = m_sleigh->instructionLength(addr);
    ...
    const std::string getRegisterName(ghidra::AddrSpace* as, ghidra::uintb off, ghidra::int4 size)
    {
